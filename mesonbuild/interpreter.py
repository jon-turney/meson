...
external dependencies (including libraries) must go to "dependencies".''')
    ...
    def func_project(self, node, args, kwargs):
        ...
        self.add_languages(proj_langs, True, True)
        self.add_languages(proj_langs, True, False)
        self.set_backend()
        if not self.is_subproject():
            self.check_stdlibs()
    ...
    def func_add_languages(self, node, args, kwargs):
        ...
        if disabled:
            ...
            return False
        if 'native' in kwargs:
            use_native = kwargs.get('native')
            if not isinstance(use_native, bool):
                raise InvalidArguments('Argument to "native" must be a boolean.')
            return self.add_languages(args, required, use_native)
        else:
            # absent 'native' means 'both' for backwards compatibility
            mlog.warning('add_languages is missing native:, assuming languages are required for both host and build.',
                         location=self.current_node)
            return (self.add_languages(args, required, True) and
                    self.add_languages(args, required, False))
    ...
    def add_languages(self, args: T.Sequence[str], required: bool, native: bool) -> bool:
        if native:
